...
#else
#  include <termios.h>
// SYSROOT=/home/sjc/duo/duo-buildroot-sdk/host-tools/gcc/sysroot
#  include <unistd.h>   // ~/duo/duo-buildroot-sdk/linux_5.10/arch/riscv/include/asm
#  include <wiringx.h>  // !! for milkv-duo !! ~/duo/duo-buildroot-sdk/buildroot-2021.05/package/wiringx/src
#endif
...
#define PRIMITIVE_LIST \
  ...
  X("PINMODE", PINMODE, WP = top; pop; setPinMode(WP, top); pop) \
...
#else
   digitalWrite(p, level);      // for milkv-duo !
#endif
}

static void setPinMode(int p, int mode) {
   pinMode(p, mode);			// for milkv-duo ! (0=not set, 2=input, 4=output, 8=interrupt)
}
...
static void Init(void) {
  ...
  /* --- for milkv-duo by SJC begin --- */
  if(wiringXSetup("duo", NULL) == -1) {
        wiringXGC();    printf("Error: wiringXSetup -- got NULL !!\n");
        return;
    }
  printf("OK: wiringXSetup(milkv duo) is good !!\nLED pin is 25.\n");
  int DUO_LED = 25;
...
#undef X
  ...
  COLD=COLON("COLD",
    BOOT,DOTQ,"Forth for milkv duo, Thanks to CH Ting.",CR,BEGIN,QUIT,AGAIN,EXIT);
  int LINE=COLON("LINE",
  ...
  CheckSum();
#endif
  setpin(LED_PIN, 0);  // Indicate successful boot.
  run();
}
